VERSION="2.9 (Michelin HFX3 09/2023)"
#
#  History
#
...
#                                  LOGS in scriptsLOG
#  2.8               16/09/23      Compessed backup sets (not performant : 15 hours for DRP3)
#  2.9               17/09/23      UnCompessed backup sets + 32 channels by default
# ------------------ ------------- ------------------------------------------------------------------------------
#
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
#
...
#COMPRESS_CLAUSE="using compressed backupset"
COMPRESS_CLAUSE=""
...
restore  database from service '$primDbUniqueName' section size $sectionSizeRESTORE $COMPRESS_CLAUSE;
switch datafile all ;
switch tempfile all ;
} 
%%
# cp /tmp/rman1_$$.txt /tmp/restoreCmd.txt
    ...
    rman target sys/${dbPassword}@${stbyDbService} >$LOG_TMP 2>&1 @/tmp/rman1_$$.txt
