...
# se positionner de nouveau à l'environnement de STANDBY.
  ...
  then
    v=""
    echo "    - Config Broker #1 exists, but not in ASM, recreating"
  fi
  if [ "$v" = "" ]
  then
    exec_sql -verbose "sys/${dbPassword}@$primDbUniqueName as sysdba" \
  ...
  then
    v=""
    echo "    - Config Broker #2 exists, but not in ASM, recreating"
  fi
...
then
  ...
  do
    ...
    scp -o StrictHostKeyChecking=no -q $f ${targetNode1}:/tmp >/dev/null 2>&1                         \
      && { echo "OK" ; }                                          \
...
$SCRIPT -d dbName [-u primService] [-U stbyService]
         ...
         dbName       : The DB_NAME
         primService  : PRIMARY Database (db Unique Name Service)
         stbyService  : STANDBY Database (db Unique Name Service - Must exists)
         scan         : Scan Address (host:port) of Opposite DB : Defaut HPR
         Channels     : Number of channels RMAN to used : defaut 32
         -C           : Copy and migrate the database (The script will execute
...
#
#   Adresse SCAN (Par défaut, HPR) DOMAINE=même domaine que le scan.
#
# if [ "$scanOppose" = "" ]
# then
  # scanOppose="dmt01-scan:1521"
# fi
...
then
  ...
  then
    ...
    then
      ...
      res=$(exec_sql "sys/${dbPassword}@$tnsTestConnect as sysdba" "select 'X' from dual ;")
      [ "$res" != "X" ] && { dbPassword="" ; echo "Incorrect" ; echo "$res" ; die "The password of the primary database is incorrect" ; } || echo OK
    fi
  fi
fi
